...
impl ClientBuilder {
    ...
    pub async fn build(self, http: impl AsRef<str>) -> Result<Client> {
        ...
        Ok(Client {
            ...
            kanari: KanariRpcClient::new(http_client.clone()),
        })
    }
}
...
impl MoveFunctionCaller for Client {
    fn call_function(
        ...
        let function_result =
            futures::executor::block_on(self.kanari.execute_view_function(function_call))?;
    ...
    fn get_module(&self, id: &ModuleId) -> Result<Option<Vec<u8>>> {
        tokio::task::block_in_place(|| {
            Handle::current().block_on(async {
                let mut states = self.kanari.get_states(AccessPath::module(id), None).await?;
                states
                    .pop()
                    .flatten()
                    .map(|state_view| {
