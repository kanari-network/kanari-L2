...
impl StateCommitmentChain {
    /// Create a new SCC
    pub fn new(kanari_store: KanariStore, moveos_store: MoveOSStore) -> anyhow::Result<Self> {
        Self::repair_last_proposed(kanari_store.clone())?;

        let last_proposed_block_number = kanari_store.get_last_proposed()?;

        let last_proposed_block_accumulator_root: H256 = match last_proposed_block_number {
            Some(last_proposed) => {
                let last_proposed_block_state = kanari_store.get_block_state(last_proposed)?;
    ...
    fn repair_last_proposed(kanari_store: KanariStore) -> anyhow::Result<()> {
        let last_proposed_block_number = kanari_store.get_last_proposed()?;
        ...
        let background_submit_block_cursor = kanari_store.get_background_submit_block_cursor()?;
        match background_submit_block_cursor {
            Some(background_submit_block_cursor) => {
                if background_submit_block_cursor < last_proposed {
                    kanari_store.set_last_proposed(background_submit_block_cursor)?;
                }
            }
            None => {
                kanari_store.clear_last_proposed()?;
            }
        }
        Ok(())
    }
    ...
    fn get_block(&self, block_number: u128) -> anyhow::Result<Block> {
        match self.last_proposed_block_number {
            Some(last_proposed) => {
                ...
                let block_da_submit_state = self.kanari_store.get_block_state(block_number)?;
    ...
    fn get_roots(&self, tx_order: u64) -> anyhow::Result<(H256, H256)> {
        let mut ledger_tx = get_ledger_tx(self.kanari_store.clone(), tx_order)?;
    ...
    fn get_accumulator_root(&self, tx_order: u64) -> anyhow::Result<H256> {
        let ledger_tx = get_ledger_tx(self.kanari_store.clone(), tx_order)?;
    ...
    fn get_prev_accumulator_root(&self, block_number: u128) -> anyhow::Result<H256> {
        ...
        let prev_block_da_submit_state = self.kanari_store.get_block_state(prev_block_number)?;
...
fn get_ledger_tx(kanari_store: KanariStore, tx_order: u64) -> anyhow::Result<LedgerTransaction> {
