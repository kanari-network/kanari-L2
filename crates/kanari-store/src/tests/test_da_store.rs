...
async fn get_submitting_blocks() {
    ...
    let da_meta_store = kanari_store.get_da_meta_store();
...
async fn catch_up_last_tx_order() {
    ...
    run_catch_up_last_tx_order_case(0, None, 0, kanari_store.clone(), None);
    run_catch_up_last_tx_order_case(1, None, 1, kanari_store.clone(), Some(0));
    run_catch_up_last_tx_order_case(2, None, 10, kanari_store.clone(), Some(0));
    ...
    let last_block_number = kanari_store.append_submitting_block(1, 3).unwrap();
    assert_eq!(last_block_number, 0);
    let last_block_number = kanari_store
