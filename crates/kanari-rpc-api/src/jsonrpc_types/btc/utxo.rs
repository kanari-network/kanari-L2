...
pub enum UTXOFilterView {
    /// Query by owner, support kanari address and bitcoin address
    ...
    pub fn into_global_state_filter(filter_opt: UTXOFilterView) -> Result<ObjectStateFilter> {
        Ok(match filter_opt {
            UTXOFilterView::Owner(owner) => ObjectStateFilter::ObjectTypeWithOwner {
                ...
                filter_out: false,
                owner: owner.0.kanari_address.into(),
            },
            UTXOFilterView::OutPoint { txid, vout } => {
                let txid = Txid::from_str(&txid)?;
                let outpoint =
                    kanari_types::bitcoin::types::OutPoint::new(txid.into_address(), vout);
...
impl UTXOObjectView {
    ...
    pub fn tx_output(&self) -> Result<TxOut> {
        // Kanari UTXO does keep the original tx output script_pubkey,
