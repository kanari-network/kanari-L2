...
impl From<IndexerEvent> for IndexerEventView {
    fn from(event: IndexerEvent) -> Self {
        IndexerEventView {
            ...
            sender: KanariAddress::from(event.sender).into(),
...
impl From<AnnotatedIndexerEvent> for IndexerEventView {
    fn from(event: AnnotatedIndexerEvent) -> Self {
        IndexerEventView {
            ...
            sender: KanariAddress::from(event.event.sender).into(),
        ...
        match event_filter {
            EventFilterView::EventTypeWithSender { event_type, sender } => {
                Self::EventTypeWithSender {
                    ...
                    sender: sender.0.kanari_address.into(),
                }
            }
            ...
            EventFilterView::Sender(address) => Self::Sender(address.0.kanari_address.into()),
