    ...
    F: Filter<T> + Clone + Send + Sync + 'static + Clone,
{
    pub fn spawn(
        ...
        let gauge = if let Some(metrics) = metrics::get_metrics() {
            metrics
                .channel_inflight
            ...
            metrics::init_metrics(&Registry::default());
            metrics::get_metrics()
                .unwrap()
                .channel_inflight
                .with_label_values(&[&channel_label])
        ...
        let (tx, rx) = metrics::metered_channel::channel(buffer, &gauge);
        let streamer = Self {
            streamer_queue: tx,
    ...
    pub fn try_send(&self, data: T) -> Result<(), KanariError> {
        self.streamer_queue.try_send(data).map_err(|e| {
            ...
            KanariError::FailedToDispatchSubscription(e.to_string())
        })
    }
}
