...
lazy_static! {
    static ref BENCH_TX_CONFIG_PATH: String = std::env::var("KANARI_BENCH_TX_CONFIG_PATH")
...
impl BenchTxConfig {
    ...
    pub fn load() -> Self {
        ...
        if let Ok(tx_type) = std::env::var("KANARI_BENCH_TX_TYPE") {
            config.tx_type = Some(tx_type.parse().unwrap());
        }
        if let Ok(btc_block_dir) = std::env::var("KANARI_BENCH_BTC_BLOCK_DIR") {
            config.btc_block_dir = Some(btc_block_dir);
        }
        if let Ok(btc_block_start_height) = std::env::var("KANARI_BENCH_BTC_BLOCK_START_HEIGHT") {
            config.btc_block_start_height = Some(btc_block_start_height.parse().unwrap());
        }
        if let Ok(btc_rpc_url) = std::env::var("KANARI_BENCH_BTC_RPC_URL") {
            config.btc_rpc_url = Some(btc_rpc_url);
        }
        if let Ok(btc_rpc_username) = std::env::var("KANARI_BENCH_BTC_RPC_USERNAME") {
            config.btc_rpc_username = Some(btc_rpc_username);
        }
        if let Ok(btc_rpc_password) = std::env::var("KANARI_BENCH_BTC_RPC_PASSWORD") {
            config.btc_rpc_password = Some(btc_rpc_password);
        }
        if let Ok(pprof_output) = std::env::var("KANARI_BENCH_PPROF_OUTPUT") {
