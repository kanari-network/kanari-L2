...
impl BitcoinRelayer {
    ...
    async fn sync_block(&mut self) -> Result<()> {
        ...
        let best_block_in_kanari = if self.buffer.is_empty() {
        ...
        let start_block_hash = match best_block_in_kanari {
            Some(best_block_in_kanari) => {
                if best_block_in_kanari.block_hash == best_block_hash_in_bitcoin.into_address() {
                    self.sync_to_latest = true;
                    return Ok(());
                }
                //We need to find the next block of the best block in kanari
                let mut best_block_header_info = self
                    .rpc_client
                    .get_block_header_info(BlockHash::from_address(best_block_in_kanari.block_hash))
                    .await?;

                // if the best block in kanari is not in the main chain, we need to find the common ancestor
            ...
            None => {
                // if the latest block in kanari is None, we start from the genesis block
