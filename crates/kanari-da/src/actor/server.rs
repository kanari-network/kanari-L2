...
impl DAServerActor {
    ...
    pub async fn get_status(&self) -> anyhow::Result<DAServerStatus> {
        let last_tx_order = if let Some(last_block_number) = self.last_block_number {
            let last_block_state = self.kanari_store.get_block_state(last_block_number)?;
        ...
        let last_avail_block_number = self.kanari_store.get_background_submit_block_cursor()?;
        let last_avail_tx_order = if let Some(last_avail_block_number) = last_avail_block_number {
            let last_block_state = self.kanari_store.get_block_state(last_avail_block_number)?;
            Some(last_block_state.block_range.tx_order_end)
        } else {
            None
        };
        ...
        tokio::spawn(async move {
            ...
            loop {
                tokio::select! {
                 ...
                 _ = ticker.tick() => {
                     match kanari_store.get_last_block_number() {
...
impl Submitter {
    async fn submit_batch_raw(
        ...
        match self.kanari_store.set_submitting_block_done(
            block_number,
            tx_order_start,
            tx_order_end,
            batch_hash,
...
impl BackgroundSubmitter {
    ...
    async fn start_job(
        ...
        for unsubmitted_block_range in unsubmitted_blocks {
            ...
            // collect tx from start to end for kanari_store
