...
impl<'a> MoveOSTestAdapter<'a> for MoveOSTestRunner<'a> {
    ...
    fn init(
        ...
        let genesis: &KanariGenesisV2 = &kanari_genesis::KANARI_LOCAL_GENESIS;
        let moveos = MoveOS::new(
            moveos_store.clone(),
            genesis_gas_parameter.all_natives(),
            MoveOSConfig::default(),
            kanari_types::framework::system_pre_execute_functions(),
            kanari_types::framework::system_post_execute_functions(),
        )
        .unwrap();
        ...
        let mut named_address_mapping = kanari_framework::kanari_framework_named_addresses()
            .into_iter()
        ...
        for (name, addr) in additional_mapping {
            if named_address_mapping.contains_key(&name) {
                panic!(
                    "Invalid init. The named address '{}' is reserved by the kanari-framework",
                    name
                )
            }
...
// pub fn kanari_framework_named_addresses_info() -> BTreeMap<String, NumericalAddress> {
...
pub fn kanari_framework_info() -> (Vec<String>, BTreeMap<String, NumericalAddress>) {
    let kanari_framework_path = PathBuf::from("../../frameworks/kanari-framework/");
    let named_addresses = resolve_package_named_addresses(kanari_framework_path.clone());

    let binding = PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        .join(kanari_framework_path.join("sources"))
        .canonicalize()
        .unwrap();
    let kanari_framework_path = binding.as_path();
    let files = find_filenames(&[kanari_framework_path], |p| {
...
pub static KANARI_NAMED_ADDRESS_MAPPING: [(&str, &str); 1] = [(
...
pub fn all_pre_compiled_libs() -> Option<FullyCompiledProgram> {
    ...
    let (kanari_framework_files, addresses) = kanari_framework_info();

    //let mut kanari_framework_named_addresses = kanari_framework_named_addresses_info();
    //kanari_framework_named_addresses.extend(addresses);

    let kanari_framework_package = PackagePaths {
        name: None,
        paths: kanari_framework_files,
