...
impl EncryptionData {
    ...
    pub fn decrypt(&self, password: Option<String>) -> Result<Vec<u8>> {
        let nonce = Base64::decode(&self.nonce)
            .map_err(|e| anyhow::Error::new(KanariError::KeyConversionError(e.to_string())))?;
    ...
    {
        let data =
            bcs::to_bytes(data).map_err(|e| KanariError::KeyConversionError(e.to_string()))?;
        ...
        T: DeserializeOwned,
    {
        ...
        let t: T =
            bcs::from_bytes(&data).map_err(|e| KanariError::KeyConversionError(e.to_string()))?;
        Ok(t)
    }
}
...
pub struct MnemonicData {
    pub addresses: Vec<KanariAddress>,
    pub mnemonic_phrase_encryption: EncryptionData,
}
