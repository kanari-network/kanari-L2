...
//Note: on kanari, we do not distinguish the Bitcoin address format,
...
pub static G_MAIN_CONFIG: Lazy<GenesisConfig> = Lazy::new(|| GenesisConfig {
    ...
    genesis_objects: vec![
        ...
        (
            KanariToBitcoinAddressMapping::genesis_with_state_root(
                H256::from_str(
            ...
            KanariToBitcoinAddressMapping::type_layout(),
        ),
    ],
...
mod tests {
    ...
    use crate::{bitcoin::multisign_account, crypto::KanariKeyPair};
    ...
    // We use the hash of "KanariNetwork" as the private key seed
    // To generate the public key, bitcoin address and kanari address for unit test usage
    #[test]
    fn test_unit_test_genesis_config() {
        let hash = sha2_256_of("KanariNetwork".as_bytes());
        let private_key = KanariKeyPair::from_secp256k1_bytes(hash.as_bytes()).unwrap();
        ...
        //println!("kanari_address: {:?}", kanari_address);
        assert_eq!(
        ...
        let multisign_address = multisign_bitcoin_address.to_kanari_address();
