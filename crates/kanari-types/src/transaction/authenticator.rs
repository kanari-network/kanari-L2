...
use crate::{
    crypto::{KanariKeyPair, Signature, SignatureScheme},
    framework::{
...
use super::KanariTransactionData;
...
impl Arbitrary for SessionAuthenticator {
    ...
    fn arbitrary_with(_args: Self::Parameters) -> Self::Strategy {
        arb_kanari_authenticator().boxed()
    }
...
impl Authenticator {
    ...
    /// Create a kanari authenticator for session key
    pub fn session(kp: &KanariKeyPair, tx_data: &KanariTransactionData) -> Self {
        SessionAuthenticator::sign(kp, tx_data).into()
    }

    /// Create a bitcoin authenticator for KanariTransaction
    /// We simulate the Bitcoin Wallet message signature
    pub fn bitcoin(kp: &KanariKeyPair, tx_data: &KanariTransactionData) -> Self {
        BitcoinAuthenticator::sign(kp, tx_data).into()
    }

    /// Create a bitcoin multisign authenticator for KanariTransaction
...
mod tests {
    ...
    proptest! {
        #[test]
        fn test_kanari_authenticator_serialize_deserialize(authenticator in any::<super::SessionAuthenticator>()) {
