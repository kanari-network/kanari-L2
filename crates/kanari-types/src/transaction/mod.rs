...
#[cfg(test)]
mod tests {
    use super::kanari::KanariTransaction;
    use crate::test_utils::random_accumulator_info;
    ...
    fn test_serialize_deserialize_roundtrip(tx: KanariTransaction) {
        let bytes = tx.encode();
        let tx2 = KanariTransaction::decode(&bytes).unwrap();
        assert_eq!(tx, tx2);
    }

    #[test]
    fn test_serialize_deserialize() {
        let tx = KanariTransaction::mock();
