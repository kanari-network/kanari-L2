...
impl FromStr for AuthenticatorOptions {
    type Err = KanariError;
    fn from_str(s: &str) -> KanariResult<Self> {
        let mut split = s.split(':');
        let auth_validator_id = split.next().ok_or_else(|| {
            KanariError::CommandArgumentError(format!("Invalid authenticator argument: {}", s))
        })?;
...
pub const KANARI_PASSWORD_ENV: &str = "KANARI_PASSWORD";
...
impl WalletContextOptions {
    pub fn build(&self) -> KanariResult<WalletContext> {
        WalletContext::new(self.config_dir.clone()).map_err(KanariError::from)
    }

    pub fn build_require_password(&self) -> KanariResult<WalletContext> {
        let mut ctx = WalletContext::new(self.config_dir.clone()).map_err(KanariError::from)?;
        if ctx.keystore.get_if_password_is_empty() {
            ...
            let password = self.password.clone().or_else(|| {
                ...
                match env::var(KANARI_PASSWORD_ENV) {
                    Ok(val) => Some(val),
                    _ => {
