...
pub struct ExecCommand {
    ...
    #[clap(
        long = "exp-root",
        help = "Path to tx_order:state_root:accumulator_root file(results from Kanari Network), for caching expected roots avoiding blocking on RPC requests"
    )]
...
impl ExecCommand {
    ...
    async fn build_exec_inner(
        ...
        let (executor, moveos_store, kanari_db) = build_executor_and_store(
        ...
        let sequenced_tx_store =
            SequencedTxStore::new(kanari_db.kanari_store.clone(), tx_anomalies.clone())?;
...
impl ExecInner {
    ...
    fn update_startup_info_after_rollback(
        ...
        let inner_store = &self.kanari_db.kanari_store.store_instance;
    ...
    async fn produce_tx(
        ...
        if let Some(rollback) = rollback_to {
            if rollback < last_partial_executed_tx_order {
                ...
                return Ok(()); // rollback done, need to restart to get new state_root for startup kanari store
            }
        };
    ...
    async fn execute(
        ...
        // because in this case, all tx have been sequenced in Kanari Network.
...
) -> anyhow::Result<(ExecutorProxy, MoveOSStore, KanariDB)> {
    let registry_service = RegistryService::default();

    let (root, kanari_db) = build_kanari_db(
        base_data_dir.clone(),
        chain_id.clone(),
        enable_rocks_stats,
        row_cache_size,
