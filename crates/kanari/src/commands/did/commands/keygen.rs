...
use clap::Parser;
use kanari_types::crypto::{self, EncodeDecodeBase64, KanariKeyPair, SignatureScheme};
...
impl CommandAction<KeygenOutput> for KeygenCommand {
    async fn execute(self) -> KanariResult<KeygenOutput> {
...
impl CommandAction<KeygenOutput> for Ed25519KeygenCommand {
    async fn execute(self) -> KanariResult<KeygenOutput> {
        let keypair = KanariKeyPair::generate_ed25519();
...
impl CommandAction<KeygenOutput> for Secp256k1KeygenCommand {
    async fn execute(self) -> KanariResult<KeygenOutput> {
        let keypair = KanariKeyPair::generate_secp256k1();
        let public_key = keypair.public();
...
impl CommandAction<KeygenOutput> for EcdsaR1KeygenCommand {
    async fn execute(self) -> KanariResult<KeygenOutput> {
        let keypair = KanariKeyPair::generate_ecdsa_r1();
        let public_key = keypair.public();
...
impl CommandAction<DidKeyOutput> for DidKeyCommand {
    async fn execute(self) -> KanariResult<DidKeyOutput> {
        // Validate key type
            ...
            || self.multibase_public_key.starts_with("zQ3s")
        {
            ...
            let _public_key = kanari_types::crypto::PublicKey::from_raw_multibase(
...
fn extract_raw_key_from_did_key(did_key_identifier: &str, key_type: &str) -> KanariResult<String> {
...
fn generate_did_key(multibase_public_key: &str, key_type: &str) -> KanariResult<String> {
