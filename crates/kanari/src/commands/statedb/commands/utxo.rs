    ...
    pub fn gen_address_mapping_data(&mut self) -> (AccountAddress, Option<AddressMappingData>) {
        ...
        match bitcoin_address {
            Some(bitcoin_address) => {
                let address = AccountAddress::from(bitcoin_address.to_kanari_address());
...
impl AddressMappingData {
    ...
    pub fn into_state(self) -> ObjectState {
        let parent_id = KanariToBitcoinAddressMapping::object_id();
        // Kanari address to bitcoin address dynamic field: name is kanari address, value is bitcoin address
