...
fn init_kanari_db(base_data_dir: Option<PathBuf>, chain_id: Option<KanariChainID>) -> KanariDB {
    let opt = KanariOpt::new_with_default(base_data_dir.clone(), chain_id.clone(), None).unwrap();
...
fn init_job(
    ...
    let kanari_db = init_kanari_db(base_data_dir, chain_id);
    let root = kanari_db
        .latest_root()
        .unwrap()
    ...
    (root, kanari_db.moveos_store, start_time)
}
...
pub fn parse_states_csv_fields(line: &str) -> Result<(String, String)> {
    ...
    if str_list.len() != 2 {
        return Err(Error::from(KanariError::from(Error::msg(format!(
            "Invalid csv line: {}",
            line
        )))));
    }
