...
pub struct GenesisVerifyCommand {
    #[clap(long, short = 'i')]
    /// utxo source data file. like ~/.kanari/local/utxo.csv or utxo.csv
    ...
    #[clap(long)]
    /// ord source data file. like ~/.kanari/local/ord or ord, ord_input must be sorted by sequence_number
...
fn verify_reverse(
    ...
    ExportCommand::export_object(
        &moveos_store_arc,
        root_state_root,
        KanariToBitcoinAddressMapping::object_id(),
        &mut writer,
    )?;
    writer.flush()?;
    ExportCommand::export_object(
...
fn verify_utxo(
    ...
    let act_address_mapping_state = resolver
        .get_field_at(
            root.state_root(),
            &KanariToBitcoinAddressMapping::object_id().field_key(),
        )
        .unwrap()
        .unwrap();
