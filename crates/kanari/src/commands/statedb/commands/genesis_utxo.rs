...
pub struct GenesisUTXOCommand {
    ...
    #[clap(long, short = 'i')]
    /// import from utxo_source. like ~/.kanari/local/utxo.csv or utxo.csv
...
impl GenesisUTXOCommand {
    pub async fn execute(self) -> KanariResult<()> {
        ...
        let moveos_store = kanari_db.moveos_store;
...
pub(crate) fn produce_address_map_updates(
    ...
    loop {
        ...
        let mut kanari_to_bitcoin_mapping_updates = UpdateSet::new();
        ...
        for line in reader.by_ref().lines().take(batch_size) {
            ...
            if let Some(address_mapping_data) = address_mapping_data {
                ...
                {
                    kanari_to_bitcoin_mapping_updates.put(field_key, object_state);
                }
            }
        }
        println!(
            "{} addr_mapping updates produced, cost: {:?}",
            kanari_to_bitcoin_mapping_updates.len(),
            loop_start_time.elapsed(),
        );

        if kanari_to_bitcoin_mapping_updates.is_empty() {
            break;
        }
        addr_tx
            .send(kanari_to_bitcoin_mapping_updates)
...
pub(crate) fn apply_address_updates(
    ...
    let mut kanari_to_bitcoin_address_mapping_state_root = *GENESIS_STATE_ROOT;
    let mut last_kanari_to_bitcoin_address_mapping_state_root =
    ...
    while let Ok(update_set) = rx.recv() {
        ...
        let mut kanari_to_bitcoin_address_mapping_tree_change_set = apply_fields(
            &moveos_store,
            kanari_to_bitcoin_address_mapping_state_root,
            update_set,
        )
        .unwrap();
        nodes.append(&mut kanari_to_bitcoin_address_mapping_tree_change_set.nodes);
        let gen_nodes_cost = gen_nodes_start.elapsed();

        kanari_to_bitcoin_address_mapping_state_root =
            kanari_to_bitcoin_address_mapping_tree_change_set.state_root;
    ...
    println!(
        "genesis kanariToBitcoinAddressMapping object updated, state_root: {:?}, count: {}",
        kanari_to_bitcoin_address_mapping_state_root, address_mapping_count
    );
}
