...
) -> KanariResult<TransactionFilterView> {
    if let Some(sender) = options.sender {
        let kanari_address_view: KanariAddressView = sender
            .into_kanari_address(&context.address_mapping())
            .unwrap()
            .into();

        Ok(TransactionFilterView::Sender(kanari_address_view.into()))
        ...
        Err(KanariError::CommandArgumentError(
