...
use kanari_key::keystore::types::LocalAccount;
use kanari_types::{
    error::KanariResult,
...
impl CommandAction<Option<AccountsView>> for ListCommand {
    async fn execute(self) -> KanariResult<Option<AccountsView>> {
        ...
        let kanari_network: KanariNetwork = context
            .client_config
            .get_active_env()
            .map(|env| env.guess_network())
            .unwrap_or(KanariNetwork::from(BuiltinChainID::Local));
