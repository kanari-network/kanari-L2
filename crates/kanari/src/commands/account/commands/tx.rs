...
impl CommandAction<Option<TransactionResultView>> for TxCommand {
    async fn execute(self) -> KanariResult<Option<TransactionResultView>> {
        let context = self.context_options.build()?;
        let client = context.get_client().await?;
        let mapping = context.address_mapping();
        let kanari_address = self.address.into_kanari_address(&mapping)?;
        let kanari_address_view = KanariAddressView::from(kanari_address);
        let united_address_view = UnitedAddressView::from(kanari_address_view);
