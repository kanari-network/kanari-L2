...
use kanari_rpc_api::jsonrpc_types::{
    IndexerStateIDView, KanariAddressView, StrView, UnitedAddressView,
};
...
impl CommandAction<Option<BalancesView>> for BalanceCommand {
    async fn execute(self) -> KanariResult<Option<BalancesView>> {
        let context = self.context_options.build()?;
        let mapping = context.address_mapping();
        let address_addr = self.address.clone().into_account_address(&mapping)?;
        let kanari_address = self.address.clone().into_kanari_address(&mapping)?;
        let coin_type = self
            .coin_type
        ...
        let btc_balance = get_bitcoin_balance(&client, kanari_address.into()).await?;
...
async fn get_bitcoin_balance(
    client: &Client,
    address: KanariAddressView,
) -> KanariResult<BitcoinBalanceInfoView> {
