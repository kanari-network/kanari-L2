        ...
        for (package_address, module_names) in package_group {
            ...
            tokio::task::block_in_place(|| {
                Handle::current().block_on(async {
                    let states = self.client.kanari.get_states(access_path, None).await?;
...
impl CommandAction<ExecuteTransactionResponseView> for Publish {
    async fn execute(self) -> KanariResult<ExecuteTransactionResponseView> {
        ...
        let pkg_address = if !empty_modules {
            ...
            return Err(KanariError::MoveCompilationError(format!(
...
impl Publish {
    ...
    async fn publish_package(
        ...
        match &self.tx_options.authenticator {
            Some(authenticator) => {
                let tx = KanariTransaction::new(tx_data, authenticator.clone().into());
