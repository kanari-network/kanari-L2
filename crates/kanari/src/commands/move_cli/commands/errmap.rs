            ...
            Some(prefix) => {
                if prefix == "Error" {
                    ...
                    return Err(KanariError::CommandArgumentError(
                                "Invalid error prefix. Use --error-prefix \"E\" for move-stdlib, --error-prefix \"Error\" for moveos-stdlib and kanari-framework, etc.".to_owned(),
                            ));
                }
            }
            None => {
                return Err(KanariError::CommandArgumentError(
                            "Error prefix not provided. Use --error-prefix \"E\" for move-stdlib, --error-prefix \"Error\" for moveos-stdlib and kanari-framework, etc.".to_owned(),
                        ));
            }
        }
