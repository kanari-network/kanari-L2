...
pub struct RunFunction {
    ...
    /// Example: `0x42::message::set_message`, `kanari_framework::empty::empty`
    #[clap(long)]
    pub function: ParsedFunctionId,

    /// TypeTag arguments separated by spaces.
    ///
    /// Example: `0x1::M::T1 0x1::M::T2 kanari_framework::empty::Empty`
    #[clap(
        long = "type-args",
    ...
    ///
    /// Example: kanari move run --function 0x3::MODULE::FUNCTION --args u64:15 --args @0x42
    ///
    ...
    async fn execute(self) -> KanariResult<ExecuteTransactionResponseView> {
        ...
        if self.dry_run {
            let kanari_tx_data = context
                ...
                .await?;
            let dry_run_result =
                dry_run_tx_locally(context.get_client().await?, kanari_tx_data).await?;
        ...
        let result = match (self.tx_options.authenticator, self.tx_options.session_key) {
            (Some(authenticator), _) => {
                let tx_data = context
                    ...
                    .await?;
                //TODO the authenticator usually is associated with the KanariTransactionData
