...
pub struct RebuildCommand {
    #[clap(long, short = 'i')]
    /// import an input file. like ~/.kanari/local/indexer.csv or indexer.csv
    pub input: PathBuf,
...
impl RebuildCommand {
    pub async fn execute(self) -> KanariResult<()> {
        ...
        let _ = produce_updates_thread
            .await
            .map_err(|_e| KanariError::from(Error::msg("Produce updates error".to_string())))?;
        let _ = apply_updates_thread
            .await
            .map_err(|_e| KanariError::from(Error::msg("Apply updates error ".to_string())))?;
