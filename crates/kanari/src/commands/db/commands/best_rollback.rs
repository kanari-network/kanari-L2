...
pub struct BestRollbackCommand {
    #[clap(
        long = "da-url",
        help = "open-da rpc url. e.g., https://storage.googleapis.com/kanari-openda-testnet/31e8fe04 for testnet, https://storage.googleapis.com/kanari-openda-main/527d69c3 for mainnet"
    )]
    pub da_url: String,

    #[clap(
        long = "last-block-number",
        help = "last_avail_block_number in da_info, got by `kanari rpc request --method kanari_status`"
    )]
    ...
    pub async fn execute(self) -> KanariResult<()> {
        ...
        if !found_best_tx_order {
            ...
            if first_mismatched_block_height != last_matched_block_height + 1 {
                return Err(KanariError::from(anyhow::anyhow!(
