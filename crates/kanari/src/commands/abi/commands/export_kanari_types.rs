...
#[derive(Debug, Parser)]
pub struct ExportKanariTypesCommand {
    #[clap(flatten)]
    pub context_options: WalletContextOptions,

    #[clap(long, default_value = "./target/kanari_types.yaml")]
    pub file_path: String,
}

#[async_trait]
impl CommandAction<()> for ExportKanariTypesCommand {
    async fn execute(self) -> KanariResult<()> {
        export_kanari_types_yaml(&self.file_path)
    }
}
...
fn export_kanari_types_yaml(file_path: &String) -> KanariResult<()> {
    ...
    tracer.trace_type::<KanariTransaction>(&samples).unwrap();
    ...
    match tracer.registry() {
        Ok(registry) => {
            ...
            println!("export kanari types to file: {file_path} ok!");
        }
        Err(e) => {
            let msg = e.explanation();
            println!("export kanari_types.yaml error: {msg}");
        }
    }
