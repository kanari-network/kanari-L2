...
use kanari_types::bitcoin::types::{self, Header};
...
async fn test_submit_block() {
    ...
    let bitcoin_module = binding_test.as_module_binding::<kanari_types::bitcoin::BitcoinModule>();
    assert_eq!(
...
fn check_utxo(txs: Vec<Transaction>, binding_test: &binding_test::RustBindingTest) {
    ...
    let utxo_module = binding_test.as_module_binding::<kanari_types::bitcoin::utxo::UTXOModule>();
    ...
    let ord_module = binding_test.as_module_binding::<kanari_types::bitcoin::ord::OrdModule>();

    let inscriptions = txs
        .iter()
        .flat_map(|tx| {
            let txid = tx.compute_txid();
            let kanari_btc_tx = kanari_types::bitcoin::types::Transaction::from(tx.clone());
            ord_module
                .parse_inscription_from_tx(&kanari_btc_tx)
                .unwrap()
                .into_iter()
                .enumerate()
...
//RUST_LOG=debug cargo test --release --package kanari-framework-tests --lib -- --include-ignored tests::bitcoin_test::test_real_bocks
#[tokio::test]
