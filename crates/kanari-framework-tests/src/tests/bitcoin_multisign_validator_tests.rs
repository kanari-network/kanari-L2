...
async fn test_validate() {
    ...
    let kp1 = KanariKeyPair::generate_secp256k1();
    let kp2 = KanariKeyPair::generate_secp256k1();
    let kp3 = KanariKeyPair::generate_secp256k1();

    let u1 = kp1.public().bitcoin_address().unwrap().to_kanari_address();
    //let u2 = kp2.public().bitcoin_address().unwrap().to_kanari_address();
    //let u3 = kp3.public().bitcoin_address().unwrap().to_kanari_address();
    ...
    let tx_data = KanariTransactionData::new_for_test(u1, 0, action);
    let tx = tx_data.sign(&kp1);
    binding_test.execute(tx).unwrap();

    let multisign_address = bitcoin_address_from_rust.to_kanari_address();
    ...
    let gas_tx_data = KanariTransactionData::new_for_test(u1, 1, gas_action);
