...
#[tokio::test]
async fn test_session_key_kanari() {
    ...
    let session_scope = SessionScope::new(KANARI_FRAMEWORK_ADDRESS, "*", "*").unwrap();
    let app_name = MoveString::from_str("test").unwrap();
    let app_url = MoveString::from_str("https:://test.kanari.site").unwrap();
    let max_inactive_interval = 100;
    let action = kanari_types::framework::session_key::SessionKeyModule::create_session_key_action(
        app_name.clone(),
        app_url.clone(),
    ...
    let session_key_module =
        binding_test.as_module_binding::<kanari_types::framework::session_key::SessionKeyModule>();
    ...
    let tx_data = KanariTransactionData::new_for_test(sender, sequence_number + 1, action);
    let tx = keystore
    ...
    let tx_data =
        KanariTransactionData::new_for_test(sender, sequence_number + 3, update_time_action);
    let tx = keystore.sign_transaction(&sender, tx_data, None).unwrap();
    binding_test.execute(tx).unwrap();

    let action = MoveAction::new_function_call(Empty::empty_function_id(), vec![], vec![]);
    let tx_data = KanariTransactionData::new_for_test(sender, sequence_number + 4, action);
    let tx = keystore
        .sign_transaction_via_session_key(&sender, tx_data, &session_auth_key, None)
        .unwrap();
