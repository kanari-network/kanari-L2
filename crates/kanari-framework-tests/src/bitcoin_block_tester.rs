...
impl BitcoinBlockTester {
    pub fn new(height: u64) -> Result<Self> {
        ...
        let network = KanariNetwork::new(chain_id, genesis_config);
        ...
        binding_test.get_kari(binding_test.sequencer, U256::from(100000000000000u64))?;
        Ok(Self {
            genesis,
            binding_test,
            executed_block: None,
    ...
    pub fn execute(&mut self) -> Result<()> {
        ...
        block_data.expect_inscriptions = block_data
            ...
            .iter()
            .filter_map(|event| {
                if let kanari_ord::event::Event::InscriptionCreated { inscription_id, .. } = event {
                    Some(*inscription_id)
                } else {
                    None
                }
    ...
    pub fn verify_inscriptions(&self) -> Result<()> {
        ...
        let inscription_created_events_from_ord = block_data
            ...
            .iter()
            .filter_map(|event| {
                if matches!(event, kanari_ord::event::Event::InscriptionCreated { .. }) {
                    Some(event.clone())
                } else {
                    None
                }
            ...
            .zip(inscription_created_events_from_move.iter())
        {
            match (event_from_ord, event_from_move) {
                (
                    kanari_ord::event::Event::InscriptionCreated {
                        charms,
                        inscription_id,
                        location,
                        ..
        ...
        for event in inscription_transferred_events_from_move {
            let find = block_data.events_from_ord.iter().find(|ord_event| {
                if let kanari_ord::event::Event::InscriptionTransferred {
                    inscription_id,
                    new_location,
                    old_location,
                    ..
    ...
    pub fn verify_bbn_stake(&self) -> Result<()> {
        ...
        for event in bbn_staking_events {
            ...
            let outpoint = kanari_types::bitcoin::types::OutPoint::new(txid, staking_output_index);
...
pub struct BlockData {
    ...
    pub events_from_ord: Vec<kanari_ord::event::Event>,
...
impl TesterGenesisBuilder {
    ...
    pub async fn add_block(mut self, block_height: u64) -> Result<Self> {
        ...
        let ord_events = match &self.ord_event_dir {
            None => vec![],
            Some(ord_event_dir) => {
                let ord_events = kanari_ord::event::load_events(
        ...
        for tx in &block.txdata {
            for input in tx.input.iter() {
                ...
                let kanari_pre_output: kanari_types::bitcoin::types::TxOut =
