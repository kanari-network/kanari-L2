...
pub fn get_data_dir() -> DataDirPath {
    match env::var("kanari_TEST_DATA_DIR") {
        Ok(path_str) => {
...
impl RustBindingTest {
    ...
    pub fn new_with_network(mut network: KanariNetwork) -> Result<Self> {
        ...
        let root = genesis.init_genesis(&kanari_db)?;
        ...
        Ok(Self {
            ...
            opt,
            root,
            sequencer: sequencer.to_kanari_address().into(),
    ...
    pub fn sequencer_kp(&self) -> &KanariKeyPair {
        &self.kp
    }
    ...
    pub fn resolver(&self) -> RootObjectResolver<MoveOSStore> {
        RootObjectResolver::new(self.root.clone(), &self.kanari_db.moveos_store)
    }
    ...
    pub fn kanari_db(&self) -> &KanariDB {
        &self.kanari_db
    }

    pub fn get_kari(&mut self, addr: AccountAddress, amount: U256) -> Result<()> {
        // transfer KARI from kanari dao account to addr
        let function_call =
            TransferModule::create_transfer_coin_action(KARI::struct_tag(), addr, amount);
        let sender = self
            .network
            .genesis_config
            .kanari_dao
        ...
        //KanariDao is a multisign account, so we need to sign the tx with the multisign account
        ...
        let tx = KanariTransaction::new(tx_data, authenticator);
        self.execute(tx)?;
        Ok(())
    }

    //TODO let the module bundle to execute the function
    pub fn execute(&mut self, tx: KanariTransaction) -> Result<ExecuteTransactionResult> {
    ...
    pub fn execute_l1_block(&mut self, l1_block: L1BlockWithBody) -> Result<Vec<L1Transaction>> {
        ...
        if l1_block.block.chain_id.is_bitcoin() {
            let block =
                bcs::from_bytes::<kanari_types::bitcoin::types::Block>(&l1_block.block_body)?;
    ...
    pub fn execute_as_result(&mut self, tx: KanariTransaction) -> Result<ExecuteTransactionResult> {
