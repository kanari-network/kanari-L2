...
const KANARI_RPC_ATTRS: [&str; 2] = ["deprecated", "version"];
...
pub fn open_rpc(attr: TokenStream, item: TokenStream) -> TokenStream {
    ...
    let methods = rpc_definition.methods.iter().map(|method: &Method|{
        ...
        if method.is_pubsub {
            quote! {
                let mut inputs: Vec<kanari_open_rpc::ContentDescriptor> = Vec::new();
                #(#inputs)*
    ...
    quote! {
        ...
        impl #open_rpc_name {
            pub fn module_doc() -> kanari_open_rpc::Module{
                let mut builder = kanari_open_rpc::RpcModuleDocBuilder::default();
                #(#methods)*
                #(#routes)*
                builder.build()
            }
                ...
                find_attr(&mut method.attrs, "method")
            {
                ...
                if let Some(version_attr) = attributes.find("version") {
                    if let (Some(token), Some(version)) = (&version_attr.token, &version_attr.value)
                    {
                        ...
                        attr.tokens = remove_kanari_rpc_attributes(attributes);
                        continue;
                    }
                }
                attr.tokens = remove_kanari_rpc_attributes(attributes);
...
// Remove kanari rpc specific attributes.
fn remove_kanari_rpc_attributes(attributes: Attributes) -> TokenStream2 {
    let attrs = attributes
        .attrs
        .into_iter()
        .filter(|r| !KANARI_RPC_ATTRS.contains(&r.key.to_string().as_str()))
