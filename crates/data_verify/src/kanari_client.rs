...
pub fn query_inscription(
    ...
    let kanari_cmd = format!(
        // "{}ord decode --txid {} --compact | jq -r '.inscriptions[0].body' | xxd -r -p",
        "{}kanari rpc request --method btc_queryInscriptions --params '[{}, null, \"1\", true]' | jq -r '.data[0]' ",
        config.ord_cli_path, query_param
    );

    let output = Command::new("sh").arg("-c").arg(kanari_cmd).output()?;

    // println!("Kanari command output: {:?}", output);
