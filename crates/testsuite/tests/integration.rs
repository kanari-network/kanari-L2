...
const RPC_USER: &str = "kanariuser";
const RPC_PASS: &str = "kanaripass";
...
impl Default for World {
    fn default() -> Self {
        ...
        World {
            opt: KanariOpt::new_with_temp_store().expect("new kanari opt should be ok"),
            docker: Cli::default(),
...
async fn start_server(w: &mut World, _scenario: String) {
    ...
    let kp: KanariKeyPair = KanariKeyPair::generate_secp256k1();
...
async fn run_cmd(world: &mut World, args: String) {
    let config_dir = world.opt.base().base_data_dir().join(KANARI_CONFIG_DIR);
...
async fn run_cli_cmd(config_dir: &Path, tpl_ctx: &mut TemplateContext, mut args: Vec<String>) {
    ...
    args.insert(0, "kanari".to_owned());
    args.push("--config-dir".to_owned());
    args.push(config_dir.to_str().unwrap().to_string());
    let opts: KanariCli = KanariCli::parse_from(args);
    let ret = kanari::run_cli(opts).await;
