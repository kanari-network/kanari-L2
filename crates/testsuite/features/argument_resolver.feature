Feature: Kanari CLI argument resolver integration tests
    ...
    Scenario: argument_resolver_test
        ...
        Then cmd: "move run --function default::argument_resolver::create_object_without_store_ability --json"
        Then assert: "{{$.move[-1].execution_info.status.type}} == executed"
        Then sleep: "3"
        Then cmd: "object -o default -t {{$.account[-1].default.hex_address}}::argument_resolver::MockObject1"
        Then cmd: "move run --function default::argument_resolver::object_without_store_ability --args object:{{$.object[-1].data[0].id}}"
