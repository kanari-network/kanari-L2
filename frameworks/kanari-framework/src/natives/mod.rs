...
pub struct NativeGasParameters {
    ...
    pub ed25519: kanari_framework::crypto::ed25519::GasParameters,
...
impl FromOnChainGasSchedule for MoveOSStdlibGasParameters {
    fn from_on_chain_gas_schedule(gas_schedule: &BTreeMap<String, u64>) -> Option<Self> {
        Some(Self {
            ...
            .unwrap(),
            ability: FromOnChainGasSchedule::from_on_chain_gas_schedule(gas_schedule).unwrap(),
        })
    }
}
...
impl ToOnChainGasSchedule for MoveOSStdlibGasParameters {
    fn to_on_chain_gas_schedule(&self) -> Vec<(String, u64)> {
        ...
        entires.extend(self.ability.to_on_chain_gas_schedule());
        entires
    }
}
...
impl InitialGasSchedule for MoveOSStdlibGasParameters {
    fn initial() -> Self {
        Self {
            ...
            ability: InitialGasSchedule::initial(),
        }
    }
}
...
impl NativeGasParameters {
    pub fn zeros() -> Self {
        Self {
            ...
            ed25519: kanari_framework::crypto::ed25519::GasParameters::zeros(),
...
pub fn all_natives(gas_params: NativeGasParameters) -> NativeFunctionTable {
    ...
    // kanari_framework natives
    add_natives!(
        "ed25519",
        kanari_framework::crypto::ed25519::make_all(gas_params.ed25519)
    );
    add_natives!(
        "ecdsa_k1",
        kanari_framework::crypto::ecdsa_k1::make_all(gas_params.ecdsa_k1)
    );
    add_natives!(
        "ecdsa_r1",
        kanari_framework::crypto::ecdsa_r1::make_all(gas_params.ecdsa_r1)
    );
    add_natives!(
        "rs256",
        kanari_framework::crypto::rs256::make_all(gas_params.rs256)
    );
    add_natives!(
        "bitcoin_address",
        kanari_framework::bitcoin_address::make_all(gas_params.bitcoin_address)
    );

    let kanari_native_fun_table = make_table_from_iter(KANARI_FRAMEWORK_ADDRESS, natives);
    native_fun_table.extend(kanari_native_fun_table);

    native_fun_table
}

#[derive(Clone, Debug)]
pub struct KanariGasParameters {
    pub kanari_framework_gas: NativeGasParameters,
    pub instruction_gas: InstructionParameter,
    pub storage_gas: StorageGasParameter,
    pub misc_gas: AbstractValueSizeGasParameter,
}

impl InitialGasSchedule for KanariGasParameters {
    fn initial() -> Self {
        Self {
            kanari_framework_gas: InitialGasSchedule::initial(),
...
impl ToOnChainGasSchedule for KanariGasParameters {
    fn to_on_chain_gas_schedule(&self) -> Vec<(String, u64)> {
        let mut entires = self.kanari_framework_gas.to_on_chain_gas_schedule();
...
impl FromOnChainGasSchedule for KanariGasParameters {
    fn from_on_chain_gas_schedule(gas_schedule: &BTreeMap<String, u64>) -> Option<Self> {
        Some(Self {
            kanari_framework_gas: FromOnChainGasSchedule::from_on_chain_gas_schedule(gas_schedule)
                .unwrap(),
...
impl KanariGasParameters {
    pub fn zeros() -> Self {
        Self {
            kanari_framework_gas: NativeGasParameters::zeros(),
