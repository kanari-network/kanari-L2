...
module kanari_framework::account_coin_store_test{
    use std::option;
    use std::signer;
    use std::string;
    ...
    use kanari_framework::account_coin_store::{Self, transfer, withdraw, withdraw_extend,
    ...
    #[test(source = @kanari_framework, destination = @0x55)]
    fun test_end_to_end(
        source: signer,
        destination: signer,
    ) {
    ...
    #[test(framework = @kanari_framework)]
    fun test_accept_twice_should_not_fail(framework: signer) {
        ...
        do_accept_coin<FakeCoin>(&framework);
        assert!(is_accept_coin<FakeCoin>(@kanari_framework), 1);
        object::transfer(coin_info_obj, @kanari_framework);
    }

    #[test(source1 = @0x33, source2 = @0x66)]
    #[expected_failure(abort_code = 1, location = kanari_framework::account_coin_store)]
