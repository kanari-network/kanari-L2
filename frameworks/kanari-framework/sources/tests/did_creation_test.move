...
module kanari_framework::did_creation_test {
    use kanari_framework::did;
    use kanari_framework::did_test_common;
    use std::string;
    use std::option;
    use std::vector;
    ...
    fun test_create_did_for_self_success() {
        ...
        let empty_dids = did::get_dids_by_controller_string(string::utf8(b"did:kanari:nonexistent"));
        ...
        let expected_did_string = string::utf8(b"did:kanari:");
        ...
        let expected_controller = did::new_kanari_did_by_address(creator_address);
    ...
    fun test_did_string_parsing() {
        // Test valid did:kanari format
        let kanari_did_string = string::utf8(b"did:kanari:bc1qw508d6qejxtdg4y5r3zarvary0c5xw7kv8f3t4");
        let parsed_kanari_did = did::parse_did_string(&kanari_did_string);
        let formatted_kanari_did = did::format_did(&parsed_kanari_did);
        assert!(formatted_kanari_did == kanari_did_string, 2001);
    ...
    fun test_did_registry_initialization() {
        ...
        let empty_controller_did = string::utf8(b"did:kanari:nonexistent");
    ...
    fun test_mock_session_key_and_bitcoin_address() {
        use kanari_framework::auth_validator;
        use kanari_framework::session_key;
        ...
        assert!(!kanari_framework::bitcoin_address::is_empty(&bitcoin_address), 5005);
        ...
        assert!(!kanari_framework::bitcoin_address::is_empty(&random_bitcoin_address), 5007);
    }
    ...
    fun test_input_validation_did_string_parsing() {
        // Test valid did:kanari format
        let valid_kanari_did = string::utf8(b"did:kanari:bc1qw508d6qejxtdg4y5r3zarvary0c5xw7kv8f3t4");
        let parsed_kanari = did::parse_did_string(&valid_kanari_did);
        let formatted_kanari = did::format_did(&parsed_kanari);
        assert!(formatted_kanari == valid_kanari_did, 13001);
    ...
    fun test_did_formatting_edge_cases() {
        ...
        let long_did = did::new_did_from_parts(string::utf8(b"kanari"), long_identifier);
        let formatted_long = did::format_did(&long_did);
        let expected_long = string::utf8(b"did:kanari:");
        ...
        let special_did = did::new_did_from_parts(string::utf8(b"kanari"), special_identifier);
        let formatted_special = did::format_did(&special_did);
        let expected_special = string::utf8(b"did:kanari:");
