...
module kanari_framework::transaction_validator {
    use std::option;
    ...
    use kanari_framework::account_authentication;
    use kanari_framework::auth_validator::{Self, TxValidateResult};
    ...
    use kanari_framework::transaction::{Self, TransactionSequenceInfo};
    use kanari_framework::session_validator;
    use kanari_framework::bitcoin_validator;
    ...
    public(friend) fun validate(
        ...
        let gas_balance = kari::balance(sender);
    ...
    /// This function is for Kanari to auto create account and address mapping.
    ...
    fun pre_execute(
        ...
        let max_gas_amount = tx_context::max_gas_amount();
        let gas = transaction_fee::calculate_gas(max_gas_amount);
        let kari = kari::deduct_gas(gas_payment_account, gas);
        transaction_fee::deposit_fee(kari);
    }

    /// Transaction post_execute function.
    /// Execute after the transaction is executed, automatically called by the MoveOS VM.
    /// This function is for Kanari to update the sender's sequence number and pay the gas fee.
    fun post_execute(
    ) {
        let sender = tx_context::sender();
        ...
        let remaining_kari = transaction_fee::distribute_fee(paid_gas, gas_used_after_scale, contract_address, sequencer_address);
        if (coin::value(&remaining_kari) > 0) {
            account_coin_store::deposit(gas_payment_account, remaining_kari);
        }else{
            coin::destroy_zero(remaining_kari);
        }
    }
}