...
module kanari_framework::did {
    ...
    /// Verification method type is not supported for Kanari session key linkage (e.g., not Ed25519)
    ...
    /// Failed to register key with the Kanari session key module
    ...
    struct DID has store, copy, drop {
        method: String,     // DID method (e.g., "kanari")
    ...
    public(friend) fun genesis_init(){
        ...
        object::transfer_extend(registry_object, @kanari_framework);
    }
    ...
    fun create_did_object_internal(
        creator_account_signer: &signer,    // The Kanari account signer creating this DID Object (pays gas)
        doc_controller: DID,                // DID Document controller field value (NIP-1)
                                             // e.g.: User self-creation: ["did:kanari:<user_addr>"]
                                             // CADOP scenario: ["did:key:<user_pk_multibase>"]

        // User's initial verification method (VM) - usually the main key associated with the new did:kanari:<addr>
        ...
        let did = new_kanari_did_by_address(did_address);
    ...
    public entry fun create_did_object_for_self_entry(
        creator_account_signer: &signer,        // User's own Kanari account signer
    ...
    public fun create_did_object_for_self(
        ...
        let creator_did = new_kanari_did_by_address(creator_address);
    ...
    fun verify_public_key_matches_account(
        ...
        let kanari_address_from_bitcoin = bitcoin_address::to_kanari_address(&bitcoin_address);
        assert!(
            kanari_address_from_bitcoin == account_address,
            ErrorDIDKeyControllerPublicKeyMismatch
        );
    }
    ...
    public entry fun create_did_object_via_cadop_with_did_key_entry(
        custodian_signer: &signer,              // Custodian's Kanari account, pays gas
    ...
    public fun create_did_object_via_cadop_with_did_key(
        ...
        let custodian_did = new_kanari_did_by_address(custodian_address);
    ...
    public entry fun add_to_verification_relationship_entry(
        ...
        if (relationship_type == VERIFICATION_RELATIONSHIP_AUTHENTICATION) {
            // Use the general authentication method that also registers it as a kanari session key
    ...
    /// Create a Kanari DID struct from an address
    /// This function only constructs a DID struct, it does NOT create a DID object on-chain
    public fun new_kanari_did_by_address(addr: address): DID {
        let did_identifier = address::to_bech32_string(addr);
        DID {
            method: string::utf8(b"kanari"),
    ...
    /// Private helper function to register a verification method as a Kanari session key.
    ...
    fun internal_ensure_session_key(
        ...
        let kanari_framework_scope = session_key::new_session_scope(
            @kanari_framework,
            string::utf8(b"*"),       
            string::utf8(b"*") 
        );
        let scopes_for_sk = vector[kanari_framework_scope, did_addr_scope];
