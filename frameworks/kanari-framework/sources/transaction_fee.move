...
module kanari_framework::transaction_fee {

    use moveos_std::object::{Self, Object};
    use moveos_std::core_addresses;
    use moveos_std::signer;

    use kanari_framework::coin_store::{Self, CoinStore};
    use kanari_framework::coin::{Self,Coin};
    ...
    fun borrow_mut_or_init_gas_revenue_store(addr: address): &mut Object<CoinStore<KARI>>{
        ...
        if(!object::contains_field(fee_pool_object, addr)){
            let gas_revenue_store_obj = coin_store::create_coin_store<KARI>();
    ...
    public fun get_gas_factor(): u64 {
        //TODO we should provide a algorithm to coordinate the gas factor based on the network throughput
        //https://github.com/kanari-network/kanari/issues/1733
        return 1
    }
    ...
    public(friend) fun withdraw_fee(amount: u256) : Coin<KARI> {
        ...
        coin_store::withdraw<KARI>(&mut pool.fee, amount)
    }

    public(friend) fun deposit_fee(gas_coin: Coin<KARI>) {
        let object_id = object::named_object_id<TransactionFeePool>();
        let pool_object = object::borrow_mut_object_extend<TransactionFeePool>(object_id);
        let pool = object::borrow_mut(pool_object);
        coin_store::deposit<KARI>(&mut pool.fee, gas_coin);
    }

    public(friend) fun distribute_fee(total_paid_gas: u256, gas_used: u256, contract_address: address, sequencer_address: address) : Coin<KARI> {
    ...
    public fun withdraw_gas_revenue(sender: &signer, amount: u256): Coin<KARI> {
    ...
    public fun gas_revenue_balance(addr: address): u256 {
        ...
        if(object::contains_field(fee_pool_object, addr)){
            let gas_revenue_store = object::borrow_field<TransactionFeePool, address, Object<CoinStore<KARI>>>(fee_pool_object, addr);
    ...
    fun test_distribute_fee(){
        ...
        kanari_framework::coin::init_for_testing();
        kanari_framework::kari::genesis_init(&system_signer);
        genesis_init(&system_signer);

        let gas_coin = kanari_framework::kari::mint_for_test(120);
