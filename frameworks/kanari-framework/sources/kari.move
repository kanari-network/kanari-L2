...
module kanari_framework::kari {
    ...
    use kanari_framework::coin::{Self, Coin, CoinInfo};
    ...
    friend kanari_framework::transaction_fee;

    /// KARI is the symbol of kanari Gas Coin
    //If not, we can remove `store` ability from KARI.
    ...
    public fun balance(addr: address): u256 {
        account_coin_store::balance<KARI>(addr)
    }

    fun borrow_mut_coin_info() : &mut Object<CoinInfo<KARI>> {
        let signer = signer::module_signer<KARI>();
        let coin_info_id = coin::coin_info_id<KARI>();
        object::borrow_mut_object<CoinInfo<KARI>>(&signer, coin_info_id)
    }

    fun mint(amount: u256): Coin<KARI> {
        let coin_info = borrow_mut_coin_info();
        coin::mint_extend<KARI>(coin_info, amount)
    }

    #[test_only]
    public fun mint_for_test(amount: u256) : Coin<KARI> {
        mint(amount)
    }

    public fun burn(coin: Coin<KARI>) {
        let coin_info = borrow_mut_coin_info(); 
        coin::burn_extend<KARI>(coin_info, coin);
    }

    /// deduct gas coin from the given account.
    public(friend) fun deduct_gas(addr: address, amount: u256):Coin<KARI> {
        account_coin_store::withdraw_extend<KARI>(addr, amount)
    }

    /// Mint gas coin to the given account.
    public(friend) fun faucet(addr: address, amount: u256) {
        let coin = mint(amount);
        account_coin_store::deposit_extend<KARI>(addr, coin);
    }
