...
module bitcoin_move::bitcoin{
    ...
    fun process_utxo(block_height: u64, pending_block: &mut Object<PendingBlock>, tx: &Transaction, is_coinbase: bool) : bool{
        ...
        let skip_ordinals = block_height >= ORDINALS_PAUSE_HEIGHT && network::is_mainnet() && kanari_framework::chain_id::is_main();
        if(!skip_ordinals){
    ...
    fun bind_bitcoin_address(kanari_address: address, bitcoin_address_opt: Option<BitcoinAddress>) {
        ...
        if(option::is_some(&bitcoin_address_opt)) {
            ...
            address_mapping::bind_bitcoin_address_by_system(&bitcoin_move_signer, kanari_address, bitcoin_address);
        };
    }
