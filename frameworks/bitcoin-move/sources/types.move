...
module bitcoin_move::types {
    ...
    public fun witness_tapscript(self: &Witness): Option<ScriptBuf> {
        ...
        while (idx < len) {
            ...
            if (idx == len - 1 && len >= 2 && vector::length(elem) > 0 && *vector::borrow(
                elem,
                0
    ...
    public fun txout_object_address(self: &TxOut): address {
        if (bitcoin_address::is_empty(&self.recipient_address)) {
            ...
            bitcoin_address::to_kanari_address(&self.recipient_address)
        }
    }
    ...
    fun test_block_decode() {
        ...
        let expected_prev_hash = bitcoin_hash::from_ascii_bytes(
            &b"00000000e47349de5a0193abc5a2fe0be81cb1d1987e45ab85f3289d54cddc4d"
        );
        //std::debug::print(&expected_prev_hash);
        assert!(block.header.prev_blockhash == expected_prev_hash, 1);
        let expected_hash = bitcoin_hash::from_ascii_bytes(
            &b"00000000b0c5a240b2a61d2e75692224efd4cbecdf6eaf4cc2cf477ca7c270e7"
        );
    ...
    fun test_block_header_hash() {
        ...
        let prev_blockhash = bitcoin_hash::from_ascii_bytes(
            &b"00000000000000000009d54a110cc122960d31567d3ee84a1f18a98f50591046"
        );
        let merkle_root = bitcoin_hash::from_ascii_bytes(
            &b"e1e0573e6098d8128ee859e7540f56b01fe0a33e56694df6d2fab0f96c4954b3"
        );
        ...
        assert!(
            block_hash == bitcoin_hash::from_ascii_bytes(
                &b"00000000000000000002b73f69e81b8b5e98dff0f2b7632fcb83c050c3b099a1"
            ),
            1
        );
    }
