...
module moveos_std::bech32 {
   ...
   fun test_encode_bech32m() {
      ...
      let hrp = b"kanari"; // hrp "kanari" for custom human-readable part
      let data = x"b19ced819df50b20648cfcabb75c4f216c77991f36d6"; // 22-bytes data for a Bech32m checksum

      let encoded = encode(bip, hrp, data);
      let expected_encoded = b"kanari1kxwwmqva759jqeyvlj4mwhz0y9k80xglxmtqxdfdl5";
   ...
   fun test_encode_no_checksum() {
      ...
      let encoded = encode(bip, hrp, data);
      let expected_encoded = b"kanari1g56g7lnzxvdvu6t8qtefzr3jal8tvqdrg9jy6navqu";
   ...
   fun test_decode_bech32m() {
      let hrp = b"kanari"; // hrp "kanari" for custom human-readable part
      let encoded = b"kanari1kxwwmqva759jqeyvlj4mwhz0y9k80xglxmtqxdfdl5";
      let decoded = decode(hrp, encoded);
   ...
   fun test_decode_no_checksum() {
      let hrp = b"kanari"; // hrp "kanari" for custom human-readable part
      let encoded = b"kanari1g56g7lnzxvdvu6t8qtefzr3jal8tvqdrg9jy6navqu";
      decode(hrp, encoded);
   }
   ...
   #[test]
   fun test_kanari_address(){
      let kanari_addr_str = b"kanari10lnft7hhq37vl0y97lwvkmzqt48fk76y0z88rfcu8zg6qm8qegfqx0rq2h";
      let data = decode(b"kanari", kanari_addr_str);
   ...
   fun test_address_bech32() {
      ...
      let encoded = encode(bip, hrp, data);
      let expected_encoded = b"kanari1qqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqppq6exstd";
      assert!(encoded == expected_encoded, 1001);

      let decode_data = decode(b"kanari", encoded);
