...
impl NibblePath {
    ...
    pub fn bits(&self) -> BitIterator {
        debug_assert!(self.num_nibbles <= ROOT_NIBBLE_HEIGHT); // invariant
        BitIterator {
            nibble_path: self,
...
impl<'a> NibbleIterator<'a> {
    ...
    pub fn visited_nibbles(&self) -> NibbleIterator<'a> {
        debug_assert!(self.start <= self.pos.start); // invariant
        debug_assert!(self.pos.start <= ROOT_NIBBLE_HEIGHT); // invariant
    ...
    pub fn remaining_nibbles(&self) -> NibbleIterator<'a> {
        debug_assert!(self.pos.start <= self.pos.end); // invariant
        debug_assert!(self.pos.end <= ROOT_NIBBLE_HEIGHT); // invariant
