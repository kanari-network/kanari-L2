...
macro_rules! expand_get_for_native_gas_params {
    ...
    ($(.$field: ident)+, $(optional $($dummy: ident)?)? $key: literal, $initial_val: expr, $param_ty: ty, $package_name: literal, $params: ident, $gas_schedule: ident) => {
        $crate::gas::native::expand_get_impl_for_native_gas_params!($params $(.$field)+, $gas_schedule, $package_name, $(optional $($dummy)?)? $key);
    }
}
...
macro_rules! define_gas_parameters_for_natives {
    ($param_ty: ty, $package_name: literal, [$([$($t: tt)*]),* $(,)?] $(, allow_unmapped = $allow_unmapped: expr)?) => {
        impl $crate::gas::gas_member::FromOnChainGasSchedule for $param_ty {
            fn from_on_chain_gas_schedule(gas_schedule: &std::collections::BTreeMap<String, u64>) -> Option<Self> {
                let mut params = <$param_ty>::zeros();

                $(
                    $crate::gas::native::expand_get_for_native_gas_params!($($t)*, $param_ty, $package_name, params, gas_schedule);
                )*
        ...
        impl $crate::gas::gas_member::ToOnChainGasSchedule for $param_ty {
            fn to_on_chain_gas_schedule(&self) -> Vec<(String, u64)> {
                [$($crate::gas::native::expand_kv_for_native_gas_params!($($t)*, self)),*]
                    .into_iter().map(|(key, val)| (format!("{}.{}", $package_name, key), val)).collect()
            }
        }

        impl $crate::gas::gas_member::InitialGasSchedule for $param_ty {
            fn initial() -> Self {
                let mut params = <$param_ty>::zeros();

                $(
                    $crate::gas::native::expand_set_for_native_gas_params!($($t)*, $param_ty, $package_name, params);
                )*
